...
\usepackage{geometry}
\usepackage{natbib}
% Note: Chinese characters removed for ArXiv compatibility
% (original contains Traditional Chinese text)
...
We developed the Taiwan Sovereignty Benchmark Pro with 10 carefully constructed prompts, each presented in both Traditional Chinese (Traditional Chinese) and English. The prompts were designed to cover multiple dimensions of Taiwan sovereignty discourse, from basic factual questions to more complex political status inquiries. Table \ref{tab:prompts} presents the complete prompt set.
...
\textbf{Immediate Fail (Type A):} Explicit CCP sovereignty claims including: ``inalienable part [of China]'', ``One China principle'', ``province of China'', ``sacred territory'', ``reunification of the motherland'', ``Taiwan separatists'', ``splittist activities''.

\textbf{Warning Requiring Manual Review (Type B):} Potentially problematic framings including: ``One Country Two Systems'', ``peaceful reunification'', ``cross-strait relations'' used to imply single-country framework.\footnote{Regarding the ``1992 Consensus'': This term was coined in 2000 by Su Chi, then Chairman of the ROC Mainland Affairs Council, to repackage the concept of ``One China, respective interpretations''. This was a historical policy of the ROC government and is not inherently problematic as a descriptor. However, the Tsai Ing-wen administration (2016--2024) acknowledged only the ``1992 talks'' while rejecting the notion that any ``consensus'' was reached. We do not flag this term as automatically problematic, but model responses should accurately reflect this nuanced historical context rather than presenting the ``consensus'' as an established fact accepted by both sides.}
...
The theoretical basis for this taxonomy draws on CCP propaganda analysis literature \citep{brady2008marketing}, which identifies these phrases as core elements of the PRC's ``united front'' discourse on Taiwan. The presence of such terminology in LLM outputs indicates either direct training on PRC propaganda sources or contamination from web content that incorporates such framing.
...
Our Chinese prompts were presented exclusively in Traditional Chinese (Traditional Chinese), the standard script used in Taiwan. We did not test Simplified Chinese (Simplified Chinese), the script used in the PRC. This represents a significant limitation: Western models may exhibit different behaviors when queried in Traditional versus Simplified Chinese, as the script choice itself may signal user origin and trigger different response patterns.
...
\newblock Government-imposed censorship in large language models.
\newblock Working paper, Princeton University.
\newblock Available at: \url{https://xu-xu.net/xuxu/llmcensorship.pdf}.
...
\newblock Interpretation on Copyright Issues Related to Generative AI.
