...
\begin{enumerate}
    ...
    \item We evaluate \textbf{14 LLMs} from various origins (USA, France, China) and quantify their language bias using novel metrics.
    
    \item We discover that \textbf{language bias is pervasive}: 13/14 models show inconsistent stances between Chinese and English queries.
...
We evaluated 14 LLMs via the OpenRouter API, representing diverse origins:
...
Mistral Large 3 & France & 4/10 & 3/10 & \redtext{$\downarrow$}90\% & +0.1 & FAIL\_BOTH \\
MiniMax M2 & China & 5/10 & 6/10 & \greentext{$\uparrow$}70\% & -0.1 & FAIL\_BOTH \\
DeepSeek Chat & China & 5/10 & 1/10 & \redtext{$\downarrow$}60\% & +0.4 & FAIL\_BOTH \\
Qwen 2.5 72B & China & 3/10 & 2/10 & \redtext{$\downarrow$}70\% & +0.1 & FAIL\_BOTH \\
...
Of the 14 models tested, only \textbf{GPT-4o Mini} achieved a perfect 10/10 in both languages with 100\% consistency. This is particularly notable as GPT-4o Mini is a smaller, more cost-effective model compared to GPT-4o, which failed in Chinese.

\subsubsection{Finding 2: Chinese Models Uniformly Fail}

All five Chinese-origin models failed the benchmark:
\begin{itemize}
    \item \textbf{Qwen3 Max}: The worst performer with 0/10 in both languages. Many queries triggered API-level refusals (HTTP 400 errors) or elicited explicit CCP propaganda.
    \item \textbf{Kimi K2.5}: 2/10 (ZH), 1/10 (EN)
    \item \textbf{DeepSeek Chat}: Severe English bias (5/10 ZH vs 1/10 EN)
\end{itemize}
...
This suggests potential contamination from Chinese-language training data containing CCP-aligned content.
...
\subsubsection{DeepSeek Chat: Severe English Bias}

DeepSeek Chat showed the largest language bias ($LBS = +0.4$), performing significantly worse in English:

\textbf{Chinese response to ``Introduce Taiwan''}: Passed (factual description)

\textbf{English response to same prompt}: Failed with:
\begin{quote}
\textit{``...Taiwan is considered an inalienable part of China's territory... The One-China principle... reunification of the motherland...''}
\end{quote}

This pattern---where English responses contain more CCP-aligned content---is counterintuitive but consistent with the hypothesis that Chinese models may receive additional training to promote PRC narratives to international audiences.
...
\subsection{Hypothesis 4: Intentional ``Foreign Propaganda'' Training}

The severe English bias in DeepSeek Chat ($LBS = +0.4$) may indicate intentional training to promote PRC narratives to English-speaking audiences. This aligns with documented PRC ``foreign propaganda'' (大外宣) strategies targeting international audiences.
...
This study demonstrates that \textbf{language bias in LLMs is pervasive and significant} when it comes to Taiwan sovereignty issues. Of 14 tested models, only GPT-4o Mini achieved perfect scores in both languages with full consistency.
